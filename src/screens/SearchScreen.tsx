...
export default function PublicModelProfileScreen() {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToMorePhotos({
      ...
      updateQuery: (prev, { subscriptionData }) => {
        ...
        const { photos: { data, ...otherInfo } } = prev
  ...
  const goToModel = (modelHash: string) => {
    navigation.navigate(
    ...
    <Container>
      <Header
        ...
        <Item style={{ backgroundColor: colors.white, height: 40 }}>
          ...
          <Input
            placeholder="Search" onChangeText={handleSearch} />
        </Item>
      </Header>
      ...
      <Segment style={{ backgroundColor: colors.white }}>
        {segmentOptions.map((segmentName, index) => (
          <Button
            ...
            style={{
              backgroundColor: segmentName === segmentNameChosen ? colors.pink : undefined,
              borderColor: colors.pink
            }}
            onPress={() => setSegmentNameChosen(segmentName)}>
            <Text style={{ color: segmentName === segmentNameChosen ? colors.white : colors.black }}>
              {segmentName}
            </Text>
          </Button>
        ))}
